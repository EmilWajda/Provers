...
const ParamLabel = ({ name, description }: { name: string, description?: string }) => (
    <div className="group relative w-fit mb-1">
         <label className="block text-sm font-medium text-gray-700 cursor-help underline decoration-dotted decoration-gray-400">{name}</label>
         {description && (
             ...
             <div className="flex flex-wrap gap-2">
                 {(value || []).map(v => (
                     <span key={v} className="bg-blue-100 text-blue-800 text-sm px-2 py-1 rounded-full flex items-center gap-1">
                        {v}
                        <button onClick={() => onChange(value.filter(x => x !== v))}><X size={14}/></button>
                     </span>
                 ))}
             </div>
        </div>
...
const CreateProblemModal = ({ onClose, onGenerate }: CreateProblemModalProps) => {
  ...
  if (isLoading) return <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50"><div className="bg-white p-4 rounded">Loading...</div></div>;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-96 p-6 animate-in fade-in zoom-in duration-200 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
            <div>
                ...
                >
                    {Object.keys(problemTypes).sort().map(t => (
            ...
            {currentTypeSpec && Object.entries(currentTypeSpec.params).map(([key, spec]) => {
                    ...
                    return (
                        <div key={key}>
                            ...
                            <input 
                                ...
                                onChange={e => setCurrentParams({...currentParams, [key]: spec.type === 'float' ? parseFloat(e.target.value) || 0 : parseInt(e.target.value) || 0})}
                            />
                        </div>
                    )
                }
